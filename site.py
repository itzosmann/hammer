...
MAX_CONCURRENT_REQUESTS = 500000000000
...
REQUESTS_PER_SECOND_CAP = 500000000000000

# TOTAL_REQUESTS: how many requests this run sends before stopping, so
#   you get a finite, inspectable test rather than an unbounded flood.
TOTAL_REQUESTS = 50
...
class LoadTester:
    ...
    async def _fetch(self, session, semaphore, stats: Stats, min_interval):
    ...
    async def run(self):
        ...
        print("Reminder: only run this against a site you own. Watch your "
              "server dashboard / Cloudflare Analytics while this runs, "
              "and hit Ctrl+C any time to stop.\n")
        ...
        async with aiohttp.ClientSession() as session:
            ...
            for _ in range(TOTAL_REQUESTS):
                ...
                tasks.append(asyncio.create_task(
                    self._fetch(session, semaphore, stats, min_interval)
                ))
